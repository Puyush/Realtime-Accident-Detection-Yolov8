    ...
    with col1:
        try:
            if source_img is not None:
                st.image(load_image(source_img), caption="Uploaded Image",
                         use_column_width=True, width=150)
    ...
    with col2:
        if (source_img is not None):
            ...
            results = model.predict(
                source=image, conf=confidence, iou=iou, imgsz=512)
            box = results[0].boxes
            res = check_acc(box)
        ...
        if source_video is not None:
            ...
            while vidcap.isOpened():
                ...
                if (frame_no % (int(fps / nof))) == 0:
                    results = model.predict(
                        image, conf=confidence, iou=iou, imgsz=512)
                    box = results[0].boxes
                    res = check_acc(box)
                    ...
                    image_holder.image(
                        annotated_frame, caption='Detected Image', use_column_width=True)
